...
if __name__ == "__main__":
    ...
    for i in range(1,4):
        if os.path.exists(f"Data\\{str(i).zfill(3)}.txt"):
            with open(f'Data\\{str(i).zfill(3)}.txt', 'r', encoding='utf-8-sig', newline='\r\n') as f:
                ddd=json.loads(f.read())

            for dd in ddd["content"]["list"]:
                print(f"{dd['varietyName']}（{dd['varietyTypeName']}）\t{dd['middlePrice']}{dd['meteringUnit']}")
        else:
            break
