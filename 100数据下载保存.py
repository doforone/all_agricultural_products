...
if __name__ == "__main__":
    ...
    for i in range(1,200):
        ...
        time.sleep(1.2)
        url=f"http://pfsc.agri.cn/api/priceQuotationController/pageList?key=&order="
        print(i)

        try:
            htmll0=get_htmll(urll="http://pfsc.agri.cn/price_portal/web/data-user-behavior-logs/hitsCount?moduleName=priceMarket")
            htmll=get_htmll2(urll=url,p=i)
            ooo=json.loads(htmll)
            #if ooo["content"]["list"]!=[]:
            with open(f'Data\\{i}_40.txt', 'w', encoding='utf-8', newline='\r\n') as f:
